...
def load_beemo_rows(rows: list[dict[str, object]]) -> list[BeemoRecord]:
    ...
    for row in rows:
        ...
        if human:
            records.append(BeemoRecord(human, 0, group_id, "human"))
        if expert:
            records.append(BeemoRecord(expert, 1, group_id, "expert_edit"))
        for key, model in (("llama-3.1-70b_edits", "llama-3.1-70b"), ("gpt-4o_edits", "gpt-4o")):
            records.extend(BeemoRecord(text, 1, group_id, variant) for variant, text in _llm_variants(str(row.get(key) or ""), model))
    return records
...
def write_records(splits: dict[str, list[BeemoRecord]], output_dir: Path) -> None:
    ...
    for name, records in splits.items():
        with (output_dir / f"{name}.jsonl").open("w", encoding="utf-8", newline="\n") as handle:
            for record in records:
                handle.write(json.dumps({"text": record.text, "label": record.label, "group_id": record.group_id, "variant": record.variant}, ensure_ascii=False, separators=(",", ":")) + "\n")
